    ...
    >
      ...
      <template #content-bottom>
        <div class="filters__buttons">
          <SfButton class="sf-button--full-width" @click="filter"
            >Done</SfButton
          >
          <SfButton
...
export default {
      ...
      sidebarAccordion: [
        {
          header: "Choose",
          items: [
            { label: "All", count: store.getters.products.length },
            { label: "Tools", count: "" },
            { label: "Top wear", count: "" },
            { label: "Bottom wear", count: "" },
            { label: "Accessorries", count: "" },
            { label: "Home", count: "" },
            { label: "Electronics", count: "" },
          ],
        },
      ],
      showOnPage: ["20", "40", "60"],
  ...
  methods: {
    ...
    pageChange() {
      ...
      for (let i = this.index; i < this.index + 8; i++) {
        if (i > store.getters.products.length) {
          break;
        }
        console.log(i, this.index, "i", "index");
        this.products.push(store.getters.products[i]);
      }
    },
    filter() {
      ...
      for (let i = 0; i < this.products.length; i++) {
        const price = this.products[i].price.regular.replace("$", "");
        if (this.filters.price[0].selected === true) {
          if (price <= 200) {
            test.push(this.products[i]);
          }
        } else if (this.filters.price[1].selected === true) {
          if (price <= 300) {
            test.push(this.products[i]);
          }
        }
    ...
    SortItems(item) {
      ...
      if (item.label == "All") {
        ...
        for (let i = 0; i < store.getters.products.length; i++) {
          if (store.getters.products[i].category == item.label) {
            this.products.push(store.getters.products[i]);
          }
        }
      }
    ...
    getDetails(product) {
      store.dispatch("ProductDetail", product);
  ...
  computed: {
    index() {
      let i = (this.currentPage - 1) * 8;
      return i;
    },
    paginationShow() {
      if (store.getters.products.length > 8) {
        return true;
      } else {
        return false;
      }
    ...
    totalPage() {
      if (store.getters.products.length % 8 != 0) {
        var length = store.getters.products.length / 8;
        return length;
      } else {
        let length = store.getters.products.length / 8 + 1;
        console.log("else");
        return length;
      }
    },
